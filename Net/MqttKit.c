...
//	说明：		
//==========================================================
uint8 MQTT_PacketConnect(const int8 *devid, const int8 *user, const int8 *password,
...
//	说明：		
//==========================================================
extern unsigned int Beat_fail_num;

void ALiYun_HeartBeat(void *mqtt_packet)
{
	unsigned char *dataPtr = NULL;
	MQTT_PACKET_STRUCTURE *mqttPacket = mqtt_packet;
...
//---------------------------------------------步骤三：解析返回数据-------------------------------------
	...
	{
		UsartPrintf(USART_DEBUG, "Tips:	HeartBeat is OK\r\n");
		Beat_fail_num = 0;
	}else
	{
		Beat_fail_num++;
		UsartPrintf(USART_DEBUG, "Check Device: Beat_fail_num=%d\r\n",Beat_fail_num);
	}
